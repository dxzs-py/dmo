...
const TOOL_CALL_RESULT_STATUSES = new Set([
 ...
 *   - Task 3+4 已移除 SSE 中的 tool 事件，工具事件统一通过 WebSocket 发布
 *
 * WebSocket（所有浏览器共享）：
...
export const useSyncStore = defineStore('sync', () => {
   ...
   * 工具调用事件（7 个 tool_call_*）不再依赖此集合跳过（SSE 已不推送工具事件）。
  ...
  const handleSessionEvent = async (event) => {
    ...
    await _processSessionEventOrdered(sessionId, event, async (ev) => {
  ...
  const _sessionEventQueue = new Map() // sessionId -> { expectedSeq, queue: Map, processing: boolean }
   ...
   * @param {string} sessionId
   * @param {Object} event
   * @param {Function} processor - 实际处理函数
   */
  const _processSessionEventOrdered = async (sessionId, event, processor) => {
    if (!event.seq || typeof event.seq !== 'number') {
      await processor(event)
      return
    }

    if (!_sessionEventQueue.has(sessionId)) {
      _sessionEventQueue.set(sessionId, { expectedSeq: 0, queue: new Map(), processing: false })
    }
    const state = _sessionEventQueue.get(sessionId)
    ...
    state.queue.set(event.seq, { event, processor })
      ...
      while (state.queue.size > 0) {
        ...
        if (minSeqInQueue > state.expectedSeq + 1 && state.expectedSeq > 0) {
          ...
          logger.warn(
            ...
            `expected=${gapStartSeq}, minSeq=${minSeqInQueue}`
          )
        }
  ...
  const applySessionEvent = async (event) => {
    ...
    if (typeof event.seq === 'number') {
      const prevSeq = lastSeenSeq.get(sessionId) || 0
  ...
  const handleStreamEvent = (sessionId, payload) => {
    ...
    // 不依赖 stream_event 先到达。replay 场景下 approval_pending 先到达设 INTERRUPTED，
   ...
   * task 通道同样使用 12 个独立事件类型，通过 event.type 直接区分。
   *
  ...
  const handleApprovalEvent = async (sessionId, payload, eventType, options = {}) => {
    const interruptId = payload.interrupt_id || payload.interruptId
