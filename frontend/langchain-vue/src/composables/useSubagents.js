 ...
 *
 * 职责：
 * - 通过 GET /ai-engine/subagents/?parent_thread_id=xxx&recursive=true 拉取
 *   子代理元数据（threadId/agentName/status/resultPreview/pendingInterruptInfo/
 *   assistantMessageId），服务端 BFS 单请求返回全树（含嵌套后代）。
...
/**
 * 单请求拉取父线程下的子代理全树元数据并合并进缓存。
 *
 * 请求放大根因（429）：原实现客户端递归逐层拉取——每个 parent_thread_id
 * 一次 HTTP 请求，嵌套场景单次刷新 1+N+M 个请求；4 浏览器并发接收同一
 * WS 事件流各自刷新，打满 user 限流 200/min → 429。
 *
 * 单请求方案：getSubagents(parentThreadId, true) 携带 recursive=true，
 * 服务端以 parent_thread_id 为根 BFS 一次返回全树（含任意深度嵌套后代，
 * 权威 meta：status/task/spawnToolCallId/depth），响应 list 直接合并进缓存。
 *
 * @param {string} parentThreadId - 父线程 thread_id（session_id 或 task_id）
 * @returns {Promise<Array>} 本次拉取到的子代理全树元数据列表
 */
async function _fetchSubagents(parentThreadId) {
  if (!parentThreadId) return []
  try {
    const res = await getSubagents(parentThreadId, true)
...
export function useSubagents() {
  ...
  /**
   * 拉取指定父线程下的子代理全树元数据并合并进缓存（单请求 recursive=true）。
   *
   * @param {string} parentThreadId - 父线程 thread_id（session_id 或 task_id）
   * @returns {Promise<Array>} 本次拉取到的子代理全树元数据列表
   */
