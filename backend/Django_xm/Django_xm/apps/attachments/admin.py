...
from .models import AttachmentCleanupLog, ChatAttachment, StorageAlert
    ...
    def action_index(self, request, queryset):
        ...
        for att in queryset.filter(status="active"):
            try:
                ...
                # 单个附件入库失败不阻断批量操作
                logger.warning("附件 %s 入库失败", att.id, exc_info=True)
    ...
    def action_unindex(self, request, queryset):
        ...
        for att in queryset.filter(status="indexed"):
            try:
                ...
                # 单个附件移除失败不阻断批量操作
                logger.warning("附件 %s 从向量库移除失败", att.id, exc_info=True)
    ...
    def action_delete(self, request, queryset):
        ...
        for att in queryset:
            try:
                ...
                # 单个附件删除失败不阻断批量操作
                logger.warning("附件 %s 删除失败", att.id, exc_info=True)
...
class AttachmentCleanupLogAdmin(admin.ModelAdmin):
    ...
    @admin.display(description="释放空间")
