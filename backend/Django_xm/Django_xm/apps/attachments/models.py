...
class ChatAttachment(AuditModel):
    ...
    file = models.FileField(upload_to=chat_attachment_upload_path, verbose_name="文件")
    ...
    last_accessed_at = models.DateTimeField(auto_now_add=True, verbose_name="最后访问时间")
    reference_count = models.PositiveIntegerField(default=1, verbose_name="引用计数")
    indexed_path = models.CharField(max_length=500, blank=True, default="", verbose_name="向量索引路径")
    indexed_at = models.DateTimeField(null=True, blank=True, verbose_name="入库时间")
    retention_days = models.PositiveIntegerField(null=True, blank=True, verbose_name="自定义保留天数")
    file_hash = models.CharField(max_length=64, blank=True, default="", verbose_name="文件SHA256哈希")
    ...
    def increment_reference(self):
        ChatAttachment.objects.filter(pk=self.pk).update(reference_count=models.F("reference_count") + 1)
    ...
    def is_expired(self):
        ...
        retention = self.retention_days or getattr(django_settings, "ATTACHMENT_DEFAULT_RETENTION_DAYS", 30)
...
class AttachmentCleanupLog(BaseModel):
    ...
    files_deleted = models.PositiveIntegerField(default=0, verbose_name="删除文件数")
    files_archived = models.PositiveIntegerField(default=0, verbose_name="入库文件数")
    files_skipped = models.PositiveIntegerField(default=0, verbose_name="跳过文件数")
    space_freed = models.PositiveBigIntegerField(default=0, verbose_name="释放空间(字节)")
    space_archived = models.PositiveBigIntegerField(default=0, verbose_name="入库空间(字节)")
    errors = models.JSONField(default=list, blank=True, verbose_name="错误列表")
    details = models.JSONField(default=dict, blank=True, verbose_name="详细信息")
    triggered_by = models.CharField(max_length=100, blank=True, default="system", verbose_name="触发来源")
