...
def get_max_upload_size() -> int:
    ...
    """
    return getattr(settings, "DATA_UPLOAD_MAX_MEMORY_SIZE", None) or getattr(
...
def _verify_magic_bytes(uploaded_file, ext: str) -> tuple[bool, str]:
    ...
    if kind is None:
        ...
        if ext in ("png", "jpg", "jpeg", "gif", "webp", "svg", "pdf", "docx", "xlsx"):
...
def build_admin_list(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if trashed:
        ...
        queryset = (
            ChatAttachment.all_objects.select_related("session", "session__user")
