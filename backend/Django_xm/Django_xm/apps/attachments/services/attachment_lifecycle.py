...
class AttachmentLifecycleService:
    ...
    def get_storage_stats(self) -> dict:
        attachments = ChatAttachment.objects.filter(status__in=[AttachmentStatus.ACTIVE, AttachmentStatus.INDEXED])
        ...
        trashed_size = (
            ChatAttachment.all_objects.filter(is_deleted=True).aggregate(total=models.Sum("file_size"))["total"] or 0
        )
    ...
    def _index_attachment(self, attachment: ChatAttachment, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            logger.exception(f"附件向量化入库失败 (id={attachment.id})")
            raise
    ...
    def _unindex_attachment(self, attachment: ChatAttachment) -> bool:
        try:
            ...
            embeddings = get_embeddings()
            removed = manager.remove_documents_by_filename(index_name, embeddings, attachment.original_name)
            logger.info(f"从向量索引移除 {removed} 个文档块: {attachment.original_name}")
            return True
        except Exception:
            logger.exception(f"从向量索引移除失败 (id={attachment.id})")
            return False
    ...
    def restore_from_trash(self, attachment_id: int) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.exception("恢复失败，无法访问文件路径")
