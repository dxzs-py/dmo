...
def build_multi_kb_retriever(
    ...
    Args:
        ...
        k: 每个检索器返回的文档数
        search_type: 检索策略
        weights: 可选权重列表，顺序与 knowledge_base_ids 一致。
                 None 时等权分配；总和不为 1 时自动归一化；
                 数量不匹配时回退到等权。
    ...
    for kb_id in knowledge_base_ids:
        ...
        try:
            ...
            required_dim = metadata.get("embedding_dimension")
    ...
    ensemble = create_multi_retriever(retrievers, weights=final_weights)
    logger.info(
        f"已创建联合检索器，包含 {len(retrievers)} 个知识库: {loaded_names}，"
        f"权重: {[round(w, 4) for w in final_weights]}"
    )
    return ensemble
