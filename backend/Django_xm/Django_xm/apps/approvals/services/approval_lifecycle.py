...
from Django_xm.apps.approvals.services.approval_service import (
    clean_extra_temp_keys,
    persist_and_broadcast,
    publish_tool_call_timeout_event,
    release_lock,
)
...
class ApprovalLifecycleService:
    ...
    def complete_batch(
        ...
        3. 逐个 persist_and_broadcast（已包含 Redis 同步 + 事件广播）
        4. 已终态的跳过（不重复广播）
    ...
    def _finalize_one(self, approval: Approval, final_state: str) -> None:
        ...
        复用 approval_service 的 persist_and_broadcast 确保 DB/Redis/广播三层一致。
        ...
        persist_and_broadcast(approval, final_state)
        ...
        release_lock(graph_interrupt_id or approval.interrupt_id)

        # 注意：不再重复调用 sync_approval_state_to_chat_message ——
        # persist_and_broadcast 内部已统一调用（含全字段比对幂等检查），
