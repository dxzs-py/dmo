...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--user", help="User ID or username for backup/restore (optional)")
        parser.add_argument("--file", help="Backup file path for restore/verify action")
        parser.add_argument("--output-dir", default="data/backups", help="Output directory for backups")
        parser.add_argument(
            "--keep-days", type=int, default=30, help="Number of days to keep old backups (for cleanup)"
        )
        parser.add_argument("--encrypt", action="store_true", help="Encrypt backup data (requires settings.SECRET_KEY)")
    ...
    def backup_data(self, options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Backup completed successfully! File: {backup_path}"))
    ...
    def restore_data(self, options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Restore completed! Restored: {restored_count}, Skipped: {skipped_count}")
        )
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"  - Redis cache synchronized for {redis_synced} sessions"))

        except Exception as e:
            self.stdout.write(self.style.WARNING(f"  - Redis cache synchronization failed (non-critical): {e!s}"))
    ...
    def cleanup_backups(self, options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Cleanup completed! Deleted {deleted_count} files ({size_mb:.2f} MB)"))
