    ...
    try:
        ...
        if attachment_ids:
            ...
            if built_content["type"] == "text":
                ...
                preloaded_attachment_type = "multimodal"
            logger.info(
                f"[Regenerate] 恢复附件内容: msg={prev_user_msg.id}, attachment_ids={attachment_ids}, type={preloaded_attachment_type}"
            )
...
async def stream_regenerate(
    ...
    logger.info(
        f"[Regenerate] 启动: session={session_id}, message={message.id}, regen_thread={regen_thread_id}, model={original_model}, tool_tier={data['tool_tier']}"
    )
    ...
    try:
        ...
        except Exception:  # noqa: S110  # cleanup, 异常路径中 streaming flag 清理失败可忽略
            pass
        raise
    except Exception as e:
        ...
        if is_interrupted:
            ...
            except Exception:  # noqa: S110  # cleanup, 中断路径 streaming flag 清理失败可忽略
                pass
        ...
        if not is_interrupted:
            try:
                await _cleanup_streaming_flag(message.id)
            except Exception:  # noqa: S110  # cleanup, finally 中 streaming flag 清理失败可忽略
                pass
...
async def stream_regenerate_resume(
    ...
    logger.info(
        f"[RegenerateResume] 配置: thread={regen_thread_id}, model={model_name}, tool_tier={tool_tier}, tools={selected_tools}"
    )
