...
def invalidate_system_config_cache(key: str = None):
...
class AIProvider(models.Model):
    ...
    provider = models.CharField(max_length=50, verbose_name="LangChain provider类型", help_text="传给 init_chat_model 的 model_provider 值，如 openai/anthropic/ollama/deepseek/groq")
    ...
    api_key_env = models.CharField(max_length=100, default="", blank=True, null=True, verbose_name="API Key 环境变量名", help_text="如 OPENAI_API_KEY，本地 provider 留空")
    api_key_attr = models.CharField(max_length=100, default="", blank=True, null=True, verbose_name="API Key settings 属性名", help_text="如 openai_api_key，本地 provider 留空")
    base_url_attr = models.CharField(max_length=100, default="", blank=True, null=True, verbose_name="Base URL settings 属性名")
    model_attr = models.CharField(max_length=100, default="", blank=True, null=True, verbose_name="模型名 settings 属性名")
    special_params = models.JSONField(default=dict, blank=True, verbose_name="特殊参数配置", help_text="DeepSeek thinking/reasoning_effort 等")
...
class AIModel(models.Model):
    ...
    capabilities = models.JSONField(default=list, blank=True, verbose_name="能力列表", help_text="如 ['tool_calling', 'streaming', 'deep_thinking']")
    is_enabled = models.BooleanField(default=True, verbose_name="是否启用")
...
class EmbeddingProviderConfig(models.Model):
    ...
    provider = models.ForeignKey(
        AIProvider, on_delete=models.CASCADE, related_name='embedding_configs',
        verbose_name="所属 LLM 提供商",
        help_text="Embedding 与 LLM 属于同一个 Provider，如 OpenAI 既提供 LLM 也提供 Embedding"
    )
    name = models.CharField(
        max_length=100, verbose_name="配置名称",
        help_text="便于在 Admin 列表中识别，如 'Ollama - bge-m3'、'Ollama - Qwen3-VL-Embedding'"
    )
    default_model = models.CharField(
        max_length=200, verbose_name="默认模型名",
        help_text="Ollama 用 'bge-m3'；OpenAI 用 'text-embedding-3-small'；多模态用 'MedAIBase/Qwen3-VL-Embedding:2b'"
    )
    factory_path = models.CharField(max_length=200, verbose_name="工厂函数路径", help_text="创建 Embedding 实例的 Python 函数路径，系统通过 importlib 动态调用。创建后不可修改。")
    dimension = models.IntegerField(default=0, verbose_name="输出维度", help_text="模型实际输出维度。仅 MRL 模型（nomic-embed-text / qwen3-embedding / embeddinggemma 等）可调整；非 MRL 模型此字段由原生最大维度锁定，不可修改。")
    native_max_dimension = models.IntegerField(
        default=0, verbose_name="原生最大维度",
    ...
    min_dimension = models.IntegerField(
        default=0, verbose_name="最小支持维度",
        help_text="MRL 截断下限，0 表示不支持截断（如 bge-m3 填 0；nomic-embed-text 填 64）。"
    )
    supported_params = models.JSONField(default=list, blank=True, verbose_name="支持的参数列表", help_text="如 ['batch_size', 'model']")
    is_enabled = models.BooleanField(default=True, verbose_name="是否启用")
    sort_order = models.IntegerField(default=0, verbose_name="排序权重")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新时间")
