...
def _query_amap(amap_key: str, normalized_city: str, original_city: str, extensions: str) -> str:
    ...
    for attempt_city in [normalized_city, original_city]:
        params = {"key": amap_key, "city": attempt_city, "extensions": extensions, "output": "JSON"}
...
class WeatherQueryInput(BaseModel):
    city: str = Field(description="城市名称，如'北京'、'上海'、'杭州'，或城市编码")
    extensions: Literal["base", "all"] = Field(
        default="base", description="天气类型，'base'获取实时天气，'all'获取未来天气预报"
    )
...
class WeatherQueryTool(AsyncToolMixin, BaseTool):
    ...
    description: str = (
        ...
        "不适用：数学计算、网络搜索、文件操作。"
        "参数：city-城市名称（如'北京'、'上海'、'杭州'，支持省份前缀自动去除，必填），"
        "extensions-天气类型（'base'获取实时天气，'all'获取未来3-4天预报，默认'base'）。"
        "边界：支持中国主要城市，小城市可能无法查询；优先使用高德 API，不可用时回退 Open-Meteo。"
    )
    args_schema: type[BaseModel] = WeatherQueryInput

    def _run(self, city: str, extensions: Literal["base", "all"] = "base") -> str:
