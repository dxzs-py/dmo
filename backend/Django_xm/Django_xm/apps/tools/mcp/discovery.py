...
class MCPServerDiscovery:
    ...
    def discover_from_config(self) -> List[Dict[str, Any]]:
        ...
        servers = getattr(django_settings, "MCP_SERVERS", [])
        result = []
        for srv in servers:
            entry = {
    ...
    def register_server(self, server_config: Dict[str, Any], user_id: Optional[int] = None) -> bool:
    ...
    def unregister_server(self, server_name: str, user_id: Optional[int] = None) -> bool:
        from django.conf import settings as django_settings
        system_servers = getattr(django_settings, "MCP_SERVERS", [])
