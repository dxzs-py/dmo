...
class PermissionPolicy:
    ...
    def _is_tool_allowed_by_session_mode(self, tool_name: str) -> bool:
        ...
        if tool_name in allowed:
            return True
        if tool_name in READ_ONLY_TOOLS | WRITE_TOOLS | DANGEROUS_TOOLS:
            return False
        return True
...
class PermissionService:
    ...
    def get_policy(user_id: int, session_id: Optional[str] = None) -> PermissionPolicy:
        ...
        if loop and loop.is_running():
            try:
                ...
                @sync_to_async
                def _load_from_db():
                    return UserPermissionPolicy.objects.filter(
                        user_id=user_id,
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(
                        lambda: UserPermissionPolicy.objects.filter(
                            user_id=user_id,
                            session_id=session_id or None,
                        ).first()
                    )
    ...
    @staticmethod
    async def aget_policy(user_id: int, session_id: Optional[str] = None) -> PermissionPolicy:
        cache_key = _get_cache_key(user_id, session_id)
        cached = get_cache().get(cache_key)
        if cached:
            policy = _cache_data_to_policy(cached)
        ...
        try:
            ...
            logger.warning(f"异步加载权限策略失败: {e}")

        return PermissionPolicy()

    @staticmethod
    async def aget_allowed_tool_names(user_id: int, session_id: Optional[str] = None) -> Set[str]:
        policy = await PermissionService.aget_policy(user_id, session_id)
    ...
    @staticmethod
    async def awrap_tools_with_permission(
        tools: List[BaseTool],
    ...
    @staticmethod
    def get_allowed_tool_names(user_id: int, session_id: Optional[str] = None) -> Set[str]:
        policy = PermissionService.get_policy(user_id, session_id)
        all_tools = READ_ONLY_TOOLS | WRITE_TOOLS | DANGEROUS_TOOLS
        return {
            tool_name
