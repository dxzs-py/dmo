...
def _field_is_instance(model: type[models.Model], field_name: str, field_cls: type) -> bool:
...
def _has_index(model: type[models.Model], index_name: str) -> bool:
...
class TestApprovalSourceStateCreatedIndex(unittest.TestCase):
    ...
    def test_user_state_created_index_still_exists(self) -> None:
        ...
        user_state_idx_exists = any(
            list(idx.fields) == ["user", "state", "created_at"] for idx in Approval._meta.indexes
        )
        self.assertTrue(
