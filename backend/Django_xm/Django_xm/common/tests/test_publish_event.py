...
class PublishEventChannelRoutingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_only_session_id_publishes_to_session_channel(self, mock_atomic, mock_get_redis, mock_get_layer):
    ...
    @patch("Django_xm.common.realtime_events._get_redis_client")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_only_task_id_publishes_to_task_channel(self, mock_atomic, mock_get_redis, mock_get_layer):
    ...
    @patch("Django_xm.common.realtime_events._get_redis_client")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_only_user_id_publishes_to_user_channel(self, mock_atomic, mock_get_redis, mock_get_layer):
    ...
    @patch("Django_xm.common.realtime_events._get_redis_client")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_session_and_task_dual_channel(self, mock_atomic, mock_get_redis, mock_get_layer):
    ...
    @patch("Django_xm.common.realtime_events._get_redis_client")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_all_none_no_publish(self, mock_atomic, mock_get_redis, mock_get_layer):
    ...
    @patch("Django_xm.common.realtime_events._get_redis_client")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_three_channels_broadcast(self, mock_atomic, mock_get_redis, mock_get_layer):
...
class PublishEventPayloadValidationTests(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("Django_xm.common.realtime_events._get_redis_client")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_valid_payload_publishes_normally(self, mock_atomic, mock_get_redis, mock_get_layer):
        """合法 payload → 正常发布。"""
        mock_layer = MagicMock()
        mock_layer.group_send = AsyncMock()
        mock_get_layer.return_value = mock_layer
...
class PublishEventErrorHandlingTests(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("Django_xm.common.realtime_events.get_channel_layer")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_redis_connection_failure_no_raise(self, mock_atomic, mock_get_layer):
        ...
        with (
            ...
            mock_atomic.assert_not_called()
            # 应记录 exception 日志（TRY400: except 块中 logger.error -> logger.exception）
            mock_logger.exception.assert_called()

    @patch("Django_xm.common.realtime_events._get_redis_client")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_group_send_failure_does_not_rollback_seq(self, mock_atomic, mock_get_redis):
        """group_send 失败 → 记录 error 但 seq 已持久化（不回滚）。"""
        mock_layer = MagicMock()
        mock_layer.group_send = AsyncMock(side_effect=RuntimeError("channel layer down"))

        with (
            patch("Django_xm.common.realtime_events.get_channel_layer", return_value=mock_layer),
            patch("Django_xm.common.realtime_events.logger") as mock_logger,
        ):
            # 不应抛出异常
            await realtime_events.publish_event(
            ...
            mock_atomic.assert_called_once()
            # group_send 失败应记录 exception 日志（TRY400: except 块中 logger.error -> logger.exception）
            mock_logger.exception.assert_called()
            # 验证 exception 日志包含 group_send 失败信息
            error_calls = [str(c) for c in mock_logger.exception.call_args_list]
            self.assertTrue(any("group_send" in c for c in error_calls))

    @patch("Django_xm.common.realtime_events.get_channel_layer")
    @patch("Django_xm.common.realtime_events._get_redis_client")
    async def test_atomic_publish_failure_no_raise(self, mock_get_redis, mock_get_layer):
        ...
        with (
            ...
            mock_layer.group_send.assert_not_called()
            # TRY400: except 块中 logger.error -> logger.exception
            mock_logger.exception.assert_called()
...
class PublishEventSeqTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_seq_injected_into_event(self, mock_atomic, mock_get_redis, mock_get_layer):
    ...
    @patch("Django_xm.common.realtime_events._get_redis_client")
    @patch("Django_xm.common.realtime_events._atomic_publish_event", return_value=42)
    async def test_ws_event_name_used_in_event_type(self, mock_atomic, mock_get_redis, mock_get_layer):
...
class PublishEventSyncTests(SimpleTestCase):
    ...
    def test_sync_swallows_other_exceptions(self, mock_async):
        ...
        with patch("Django_xm.common.realtime_events.logger") as mock_logger:
            ...
            # RuntimeError 被吞后应记录 exception 日志（TRY400: except 块中 logger.error -> logger.exception）
            mock_logger.exception.assert_called()
