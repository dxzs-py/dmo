...
def execute_workflow_task(self, thread_id: str, user_question: str, user_id: int | None = None):
    ...
    try:
        ...
        logger.exception(f"[Celery Workflow] 工作流执行失败：{thread_id}, 错误：")
        try:
            session = WorkflowSession.objects.filter(session_id=thread_id).first()
            if session:
                session.status = WorkflowSessionStatus.FAILED
            ...
            # 二次失败（更新会话状态）不掩盖原始工作流错误
            logger.debug("更新 WorkflowSession 状态失败")

        try:
            tracker.mark_failure(error_message=str(exc))
        except Exception:
            # tracker 更新失败不影响错误处理主流程
            logger.debug("tracker.mark_failure 失败（工作流）")
