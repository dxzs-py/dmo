...
class ResumeChatAfterTimeoutEntryLogTests(unittest.TestCase):
    ...
    def test_resume_chat_after_timeout_entry_log(self, mock_get):
        ...
        with self.assertLogs("Django_xm.tasks.approval_tasks", level="INFO") as cm:
            # 触发函数（会因 chat_session_id 缺失走完整路径）
            with patch("Django_xm.tasks.approval_tasks.Approval.objects.filter") as mock_filter:
                mock_filter.return_value.exclude.return_value.first.return_value = None
                # mock complete_approval 避免实际终态化
                with patch("Django_xm.apps.approvals.services.approval_service.complete_approval"):
...
class ResumeChatAfterTimeoutMissingSessionTests(unittest.TestCase):
    ...
    @patch("Django_xm.tasks.approval_tasks.Approval.objects.get")
    def test_resume_chat_after_timeout_missing_session_id_logs_error(self, mock_get, mock_complete):
        ...
        with patch("Django_xm.tasks.approval_tasks.Approval.objects.filter") as mock_filter:
            ...
            with self.assertLogs("Django_xm.tasks.approval_tasks", level="ERROR") as cm:
    ...
    @patch("Django_xm.apps.approvals.services.approval_service.complete_approval")
    @patch("Django_xm.tasks.approval_tasks.Approval.objects.get")
    def test_resume_chat_after_timeout_missing_session_id_no_graph_interrupt(self, mock_get, mock_complete):
...
class ResumeChatAfterTimeoutRecoverFromSiblingTests(unittest.TestCase):
    ...
    @patch("Django_xm.tasks.approval_tasks.Approval.objects.get")
    def test_resume_chat_after_timeout_recovers_session_from_sibling(self, mock_get, mock_session_filter):
        ...
        with patch("Django_xm.tasks.approval_tasks.Approval.objects.filter") as mock_filter:
            ...
            with (
                ...
                self.assertLogs("Django_xm.tasks.approval_tasks", level="INFO") as cm,
            ):
