...
class TaskFailureSignalIdempotencyTests(unittest.TestCase):
    ...
    def test_failure_signal_marks_non_terminal_record(self, mock_objects):
        ...
        args, kwargs = mock_record.mark_failure.call_args
...
class RagAddDocsTaskIdempotencyTests(unittest.TestCase):
    ...
    def test_task_skips_when_lock_held(self):
        ...
        with patch(
            ...
            'Django_xm.tasks.rag_tasks._release_rag_add_docs_lock'
        ) as mock_release, patch(
            'Django_xm.tasks.base.TrackedTask.mark_success'
        ) as mock_mark_success:
            result = add_documents_to_index_task.apply(
                args=('test-index', ['/tmp/test.pdf']),
    ...
    def test_task_releases_lock_on_failure(self):
        ...
        with patch(
            ...
            'Django_xm.tasks.rag_tasks.get_index_manager'
        ) as mock_manager, patch(
            'Django_xm.tasks.base.TrackedTask.mark_started'
        ), patch(
            'Django_xm.tasks.base.TrackedTask.mark_failure'
        ):
            ...
            result = add_documents_to_index_task.apply(
                args=('nonexistent-index', ['/tmp/nonexistent.pdf']),
            ).get()
