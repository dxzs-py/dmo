...
def _get_signal_redis_url() -> str:
    """信令 Redis URL：优先 CELERY_BROKER_URL，回退应用配置 redis_url。

    说明：复用 Celery broker 的 Redis 连接串（保证与订阅方使用同一 Redis DB，
    Pub/Sub 按 DB 隔离），仅复用配置值，深度研究执行服务不依赖 Celery 功能。
    """
    ...
    return getattr(app_cfg, "redis_url", "redis://127.0.0.1:6379/1")
...
def publish_signal(kind: str, thread_id: str, payload: dict) -> None:
    ...
    try:
        client = redis_lib.Redis.from_url(_get_signal_redis_url())
